#!/usr/bin/env python3
"""Generate CHARACTER_MAPPINGS dict entries for unmapped NOT_IMPLEMENTED characters."""
...
from card_mapping_rules import AFFINITY_WORDS, normalize_ability  # noqa: E402
from sync_card_data_from_xlsx import load_workbook, sheet_rows, gd_name  # noqa: E402

SUGGESTIONS = ROOT / "context/card_ability_mapping_suggestions.json"
OUT = ROOT / "tools/_character_mappings.json"

CHAR_ENUM = set(
    re.findall(
        r"^\t([A-Z_0-9]+),",
        Path(ROOT / "resources/CharacterData.gd")
        .read_text(encoding="utf-8")
        .split("enum AbilityType {")[1]
        .split("\n}")[0],
        re.M,
    )
)
...
def _aff(word: str, fallback: str = "") -> str:
    return AFFINITY_WORDS.get(word.lower(), fallback.upper() if fallback else word.upper())


def map_character(name: str, ability: str, affinity: str) -> dict | None:
    ab = normalize_ability(ability)
    if not ab or ab == "none":
        return {"ability_type": "NONE", "ability_params": {}}

    # ── Per-card explicit mappings (162 NOT_IMPLEMENTED characters) ──────────
    EXPLICIT: dict[str, dict] = {
        "Silver Dragon": {
            "ability_type": "OPTIONAL_CRYSTAL_PAY_ATK_BOOST",
            "ability_params": {"cost": 1000, "atk": 0},
        },
        "Armored Wolf": {
        ...
        "Nachzehrer": {
            ...
            "ability_params": {
                "max_attacks": 2,
                "requires_affinity_on_field": "ANIMA",
        ...
        "Randez the Rogue King": {
            ...
            "ability_params": {
                ...
                "atk": 100,
                "def": 100,
                "penalty_vs_affinity": {"affinity": "ANIMA", "atk": 30, "def": 30},
            },
        },
    ...
    # ── Pattern fallbacks (also used by suggest_character regex section) ─────
    m = re.search(r"\+\s*(\d+) atk&def vs (\w+)", ab)
    if m:
        return {
            "ability_type": "ATK_DEF_BONUS_VS_AFFINITY",
            "ability_params": {
                "affinity": _aff(m.group(2), affinity),
                "atk": int(m.group(1)),
                "def": int(m.group(1)),
            },
        }
    ...
    if m:
        return {
            "ability_type": "DEF_BONUS_VS_AFFINITY",
            "ability_params": {"affinity": _aff(m.group(2), affinity), "bonus": int(m.group(1))},
        }
    ...
    if "flip a coin" in ab and "defend" in ab and "attack does nothing" in ab:
        return {"ability_type": "COIN_FLIP_NULLIFY_ON_DEFEND", "ability_params": {}}

    if "unaffected by 0 cost trap" in ab or "unaffected by 0-cost trap" in ab:
        return {"ability_type": "IMMUNE_ZERO_COST_TRAPS", "ability_params": {}}

    if "not affected by trap" in ab or "immune to trap" in ab:
        return {"ability_type": "IMMUNE_TO_TRAPS", "ability_params": {}}

    if "not affected by tech" in ab or "immune to tech" in ab:
        return {"ability_type": "IMMUNE_TO_TECH_CARDS", "ability_params": {}}

    if "not destroyed by" in ab:
        m2 = re.search(r"not destroyed by [\"']?(\w+)", ab)
        if m2:
            daff = _aff(m2.group(1))
            if daff:
                return {
                    "ability_type": "ONE_USE_SURVIVE_DESTRUCTION",
    ...
    m = re.search(r"once.*\+(\d+) atk&def vs nature", ab)
    if m:
        return {
            "ability_type": "ONE_USE_TEMP_BOOST_ATTACK_AND_DEFEND",
            "ability_params": {"atk": int(m.group(1)), "def": int(m.group(1)), "vs_affinity": "NATURE", "temp": True},
        }

    if "+60 atk&def if exposed" in ab:
        return {
            "ability_type": "ATTACK_STANCE_BOOST",
            "ability_params": {"atk": 60, "def": 60, "when_exposed": True},
        }
...
def main() -> int:
    names = json.loads(SUGGESTIONS.read_text(encoding="utf-8"))["characters"]
    wb = load_workbook()
    _, units = sheet_rows(wb, "Unit")
    cards = {gd_name(c["_name"]): c for c in units}
    ...
    for name in names:
        ...
        sug = map_character(name, ab, aff)
        if not sug:
            unmapped.append(name)
            continue
        if sug["ability_type"] not in CHAR_ENUM:
            bad.append((name, sug["ability_type"]))
        mappings[name] = sug

    OUT.write_text(json.dumps(mappings, indent="\t") + "\n", encoding="utf-8")
    print(f"Wrote {len(mappings)} mappings to {OUT.relative_to(ROOT)}")
    print(f"Unmapped: {len(unmapped)}")
    for n in unmapped:
        print(f"  {n}")
    if bad:
        print(f"Bad enum: {bad}")
    return 0
